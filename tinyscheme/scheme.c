...
#ifdef WIN32
         if(((unsigned long long)cp)%adj!=0) {
           cp=(char*)(adj*((unsigned long long)cp/adj+1));
         }
#else
...
static void check_cell_alloced(pointer p, int expect_alloced)
{
  ...
  if(typeflag(p) & !expect_alloced)
    {
      TraceLog(LOG_ERROR,  "Cell is already allocated!");
    }
  if(!(typeflag(p)) & expect_alloced)
    {
      TraceLog(LOG_ERROR, "Cell is not allocated!");
    }
...
int main(int argc, char **argv) {
  ...
  if(!scheme_init(&sc)) {
    TraceLog(LOG_ERROR, "Could not initialize!\n");
    return 2;
  }
  ...
  do {
    ...
    if(isfile && fin==0) {
      ...
      if(!isfile || fin!=stdin) {
        if(sc.retcode!=0) {
          TraceLog(LOG_ERROR,"Errors encountered reading %s\n",file_name);
        }
        if(isfile) {
          fclose(fin);
        }
